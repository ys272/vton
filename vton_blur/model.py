...
class Unet_Person_Masked(nn.Module):
    def __init__(
        ...
        layers = []
        for rep in range(level_reps):
            layers.append(ResnetBlock(dim_out if rep==0 else dim_out*2, dim_out, film_emb_dim=None))
            layers.append(Residual(PreNorm(SelfAttention(dim_out), dim_out), dim=None))
            layers.append(Residual(PreNorm(CrossAttention(dim_out, dim_out_cross_attn, dim_head=64), dim_out, dim_out_cross_attn, affine=True), dim=dim_out))
        ...
        for level_idx in range(len(level_dims)-2,-1,-1):
            ...
            for rep in range(level_reps):
                layers.append(ResnetBlock(dim_in+dim_out if rep==0 else 2*dim_out, dim_out, film_emb_dim=None))
                if level_att:
                    layers.append(Residual(PreNorm(SelfAttention(dim_out), dim_out), dim=None))
                    layers.append(Residual(PreNorm(CrossAttention(dim_out, dim_out_cross_attn, dim_head=64), dim_out, dim_out_cross_attn, affine=True), dim=dim_out))
...
class Unet_Clothing(nn.Module):
    def __init__(
        ...
        for level_idx in range(len(level_dims)-1):
            ...
            layers = []
            for rep in range(level_reps):
                layers.append(ResnetBlock(init_dim if level_idx==0 and rep==0 else dim_out, dim_out, film_emb_dim=None, clothing=True if level_idx==len(level_dims)-2 else False))
            layers.append(Downsample(dim_out, dim_next))
            self.downs.append(nn.ModuleList(layers))
        ...
        layers = []
        for rep in range(level_reps):
            layers.append(ResnetBlock(dim_out, dim_out, film_emb_dim=None, clothing=True))
        self.mid1 = nn.ModuleList(layers)
