...
if __name__ == '__main__':
    ...
    level_dims_main = (256, 480, 768)
    level_attentions = (False, True)
    level_repetitions_main = (3,4,4)
    
    model_main = Unet_Person_Masked(channels=10, init_dim=init_dim, level_dims=level_dims_main, level_attentions=level_attentions,level_repetitions = level_repetitions_main,).to(c.DEVICE)
        ...
        for epoch in range(epochs):
            for batch in train_dataloader:
                ...
                with torch.cuda.amp.autocast(dtype=torch.float16):
                    model_final_input = torch.cat((down32,masked_aug,clothing_aug,mask_coords.unsqueeze(1)), dim=1)
                    out = model_main(model_final_input, pose, noise_amount_masked)
                ...
                if batch_num % c.EVAL_FREQUENCY == 0:
                    ...
                    for model_main_,suffix in [(model_main, ''), (ema_model_main, '_ema')]:
                        ...
                        with torch.no_grad():
                          with torch.cuda.amp.autocast(dtype=torch.float16):
                            model_final_input = torch.cat((down32,masked_aug,clothing_aug,mask_coords.unsqueeze(1)), dim=1)
                            img_sequences = model_main_(model_final_input, pose, noise_amount_masked) 
